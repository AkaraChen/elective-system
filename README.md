...
- 年级顺序 `grade_order`（入学年份 / 毕业年份）与课程允许年级 `allowed_grade`
...
- 学生账号保存入学年或毕业年（由 `grade_order` 决定如何解读）
- 当前年级按本地时间 9 月 1 日切换学年计算；毕业年默认按 3 年学制反推入学年
- 课程 `allowed_grade` 为逗号分隔年级，如 `1,3`；留空表示不限
- 学生端列表和抢课接口只保留该生年级允许的课程（已选课程仍可见，便于退课）
